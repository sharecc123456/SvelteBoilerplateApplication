<script>
  ...
  export let selectOne = false;
  export let showSelectionCount = true;
  export let customButtonText = false;
  export let buttonText = "";
  export let templateFilter = "none";
  export let title = "Select Existing Templates";
  ...
  export let templates = getTemplates();
  async function getTemplates() {
    let request = await fetch(`/n/api/v1/templates?filter=${templateFilter}`);
  ...
  function cannotAddMore(templates) {
    let firstArr = templates.map((x) => x.id);
      ...
      <div class="inner-content">
        ...
        <div class="table">
          <div class="tr th">
            ...
            <div class="td date">Type</div>
            <div class="td date desktop">Date Created</div>
            <div class="td date desktop">Last Modified</div>
            <div class="td toggle" />
          </div>
        </div>
      </div>
        ...
        <div class="table">
          ...
          {:then tmpls}
            {#each tmpls as tmpl}
              ...
              {/if}
            {/each}
            {#if cannotAddMore(tmpls)}
              <div class="tr-full">
                <p>All available templates have been added to the Checklist.</p>
              </div>
            {/if}
          {:catch error}
